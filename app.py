...
# Các route giao diện bổ sung
@app.route('/profile')
def profile():
    if 'logged_in' not in session:
        return redirect(url_for('login'))
    # Dữ liệu mẫu, thực tế lấy từ session hoặc service
    user_info = session.get('user_info', {})
    user_info['email'] = user_info.get('email', 'sv@ictu.edu.vn')
...
def schedule():
    ...
    # Dữ liệu mẫu, thực tế lấy từ service
    schedule = [
        {'date': '15/08/2025', 'subject': 'Lập trình Python', 'room': 'A101', 'time': '7:00-9:30'},
        {'date': '16/08/2025', 'subject': 'Toán rời rạc', 'room': 'B202', 'time': '9:45-12:00'},
    ]
...
def lichthi():
    ...
    if 'logged_in' not in session:
        return redirect(url_for('login'))
    return render_template('lichthi.html', user_info=session.get('user_info'))
...
def diem():
    ...
    if 'logged_in' not in session:
        return redirect(url_for('login'))
    return render_template('diem.html', user_info=session.get('user_info'))
...
def dangkihoc():
    ...
    if 'logged_in' not in session:
        return redirect(url_for('login'))
    return render_template('dangkihoc.html', user_info=session.get('user_info'))

@app.route('/timkiem')
def timkiem():
    """Trang tìm kiếm lịch học"""
    if 'logged_in' not in session:
        return redirect(url_for('login'))
    return render_template('timkiem.html', user_info=session.get('user_info'))
